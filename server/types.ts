...
export interface SeniorGuardian {
  id: string;
  senior_id: string;
  guardian_id: string;
  permissions: string[];
...
export interface XpTransaction {
  id: string;
  senior_id: string;
  activity_type: 'wakeup' | 'walking' | 'breathing' | 'yoga' | 'meal' | 'medicine' | 'nap' | 'streak_bonus' | 'reward_redemption';
  xp_amount: number;
  description: string;
  created_at: string;
}
...
export interface RefillRequest {
  ...
  quantity_remaining: number;
  requested_at: string;
  status: 'requested' | 'ordered' | 'fulfilled';
  notes?: string;
}
