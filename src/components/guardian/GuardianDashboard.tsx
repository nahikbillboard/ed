...
import { Shield, CheckCircle2, AlertTriangle, Phone, MessageSquare, Pill, Footprints, Clock, Sparkles, RefreshCw, ChevronRight, Bell, AlertCircle } from 'lucide-react';
...
export const GuardianDashboard: React.FC<GuardianDashboardProps> = ({
  ...
  const routineItems = [
    ...
    { label: 'Dinner', time: senior.dinner_time, status: routine.dinner_status, val: 'Scheduled 6:30 PM' },
    { label: 'Medicine #1 (Lisinopril)', time: '8:00 AM', status: routine.medicine_status, val: 'Taken' },
    { label: 'Medicine #3 (Atorvastatin)', time: '9:00 PM', status: routine.night_medicine_status, val: 'Scheduled' },
      ...
      <div className="bg-[#FAF8F5] border border-stone-200 rounded-[32px] p-6 sm:p-8 text-stone-900 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-stone-600 text-sm sm:text-base font-normal">
            Monitored by <strong className="font-semibold text-stone-900">{senior.guardian_name}</strong> (Primary Guardian)
          </p>
        </div>
      ...
      <div className="bg-[#FAF8F5] border border-stone-200 rounded-[32px] p-6 sm:p-7 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold text-stone-700 hover:text-stone-900 bg-white hover:bg-stone-100 border border-stone-200 px-3.5 py-1.5 rounded-full transition-all shadow-xs"
          >
        ...
        <div className="bg-white border border-stone-200 rounded-[32px] p-6 shadow-xs space-y-4 flex flex-col justify-between">
          ...
          <div className="pt-4 border-t border-stone-100 grid grid-cols-2 gap-2">
            <button
              ...
              className="py-3 px-4 bg-[#FF6321] hover:bg-[#e85516] active:scale-95 text-white font-bold text-xs sm:text-sm rounded-xl flex items-center justify-center gap-2 shadow-xs transition-all"
            >
            ...
            <button
              ...
              className="py-3 px-4 bg-stone-900 hover:bg-stone-800 active:scale-95 text-white font-bold text-xs sm:text-sm rounded-xl flex items-center justify-center gap-2 shadow-xs transition-all"
            >
